...
#Split the answers into individual elements
split_answers_barriers <- strsplit(mydata_cleaned$barriers, ", ")

#Flatten the split answers into a single vector
flattened_answers_barriers <- unlist(split_answers_barriers)

#Count the frequency of each option
option_counts_barriers <- table(flattened_answers_barriers)

#Sort the options by frequency in descending order
sorted_options_barriers <- sort(option_counts_barriers, decreasing = TRUE)

#Shorten labels
barriers_options_short <- c("Time",
                      "Suitability",
                      "Unsure where",
                      "Unsure how",
                      "Health",
...
par(mar = c(8, 10, 2, 2))
par(mgp = c(8, 0, -0.4))
barplot(sorted_options_barriers, horiz = TRUE, main = "Barriers to Getting Involved", names= barriers_options_short,
        xlab = "Frequency", ylab = "Options", col = rainbow(11), las=1)

dev.off()

#Print table
print(sorted_options_barriers)
```
...
#Split the answers into individual elements
split_answers_help <- strsplit(mydata_cleaned$help, ", ")

#Flatten the split answers into a single vector
flattened_answers_help <- unlist(split_answers_help)

#Count the frequency of each option
option_counts_help <- table(flattened_answers_help)

#Sort the options by frequency in descending order
sorted_options_help <- sort(option_counts_help, decreasing = TRUE)

#Shorten labels
help_options_short <- c("Clearer descriptions",
...
par(mar = c(8, 10, 2, 2))
par(mgp = c(8, 0, -0.4))
barplot(sorted_options_help, horiz = TRUE, main = "Facilitators to Getting Involved", names= help_options_short,
        xlab = "Frequency", ylab = "Options", col = rainbow(6), las=1)

#Print table
print(sorted_options_help)
```
...
#Split the answers into individual elements
split_answers_benefits <- strsplit(mydata_cleaned$benefits, ", ")

#Flatten the split answers into a single vector
flattened_answers_benefits <- unlist(split_answers_benefits)

#Count the frequency of each option
option_counts_benefits <- table(flattened_answers_benefits)

#Sort the options by frequency in descending order
sorted_options_benefits <- sort(option_counts_benefits, decreasing = TRUE)

#Shorten labels
benefits_options_short <- c("Contact",
                      "Flexibility",
...
par(mar = c(8, 10, 2, 2))
par(mgp = c(8, 0, -0.4))
barplot(sorted_options_benefits, horiz = TRUE, main = "Benefits to Getting Involved", names= benefits_options_short,
        xlab = "Frequency", ylab = "Options", col = rainbow(6), las=1)
